...
// Query data edukasi berdasarkan id
$id = $_GET['id'];
$row = query("SELECT * FROM edukasi WHERE id = $id")[0];

// cek ketika tombol ubah sudah diklik
if (isset($_POST['ubah'])) {

  // ubah data di tabel edukasi
  if (ubah($_POST) > 0) {
    echo "<script>
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="ml-5 mt-3">
      ...
      <div class="form-group">
        <div class="row">
          ...
          <div class="col-sm-4">
            <input type="text" name="nama" class="form-control"<?= $row['judul']; ?> autofocus required>
          </div>
        </div>
      </div>
      <div class="form-group">
        <div class="row">
          ...
          <div class="col-sm-4">
            <input type="text" name="detail" class="form-control" required value="<?= $row['detail']; ?>">
          </div>
        </div>
      </div>
      <!-- gambar -->
