...
function koneksi()
{
  ...
  mysqli_select_db($conn, 'adaapa') or die("Database salah!");
  return $conn;
}
...
function hapus($id)
{
  ...
  // ambil data edukasi
  $edukasi = query("SELECT * FROM edukasi WHERE id = $id")[0];

  // hapus data gambar
  if ($edukasi["gambar"] !== 'nophoto.jpg') {
    unlink('img/' . $edukasi["gambar"]);
  }

  mysqli_query($conn, "DELETE FROM edukasi WHERE id = $id") or die(mysqli_error($conn));
  return mysqli_affected_rows($conn);
}
